...
function SourceCard({ source, children }: { source: SourceSnapshot; children?: ReactNode }) {
  return <article className="source-snapshot"><header><div><small>{source.authorType}</small><h3>{source.title}</h3><p>{source.author}</p></div><a href={source.url} target="_blank" rel="noreferrer" title="查看知乎原文"><ExternalLink /></a></header><p>{source.body}</p>{source.sample && <div className="study-sample"><BookOpen />研究对象：{source.sample}</div>}{children}<footer><ShieldCheck />限制：{source.limitations}</footer></article>;
}
...
function SnapshotWorkbench({ round, onState }: { round: RoundConfig; onState: (value: WorkbenchResult) => void }) {
  ...
  useEffect(() => {
    ...
    if (round.mode === "professional") {
      ready = Boolean(choice && boundaryAnswer === "cannot");
      const source = sources[0]; evidence = { E04: { excerpt: choice, sourceId: source?.id, sourceTitle: source?.title, sourceUrl: source?.url, limitations: source?.limitations } };
    } else if (round.mode === "comments") {
      ready = checked.includes("C1") && checked.includes("C2") && Boolean(variable);
      const source = sources[0]; evidence = { E06: { excerpt: source?.comments?.filter((item) => checked.includes(item.id)).map((item) => item.text).join(" / "), sourceId: source?.id, sourceTitle: source?.title, sourceUrl: source?.url, limitations: source?.limitations } };
  ...
  return <section className="snapshot-workbench">{sources.map((source) => <SourceCard key={source.id} source={source}>{round.mode === "professional" || round.mode === "research" ? <div className="excerpt-options">{source.excerpts?.map((excerpt) => <button key={excerpt} className={choice === excerpt ? "selected" : ""} onClick={() => setChoice(excerpt)}><CheckCircle2 />{excerpt}</button>)}</div> : null}{round.mode === "comments" && <div className="comment-list">{source.comments?.map((comment) => <button key={comment.id} className={checked.includes(comment.id) ? "selected" : ""} onClick={() => toggle(comment.id)}><span>{comment.relation === "support" ? "支持" : comment.relation === "challenge" ? "质疑" : "补充"}</span><p>{comment.text}</p><Check /></button>)}</div>}{round.mode === "comparison" && <button className={`viewpoint-select ${viewpoint === source.id ? "selected" : ""}`} onClick={() => setViewpoint(source.id)}>{viewpoint === source.id ? <CheckCircle2 /> : <i />} 这篇观点更能解释两晚差异</button>}</SourceCard>)}{round.mode === "professional" && <div className="boundary-question"><header><small>因果边界判断</small><b>这篇内容能否直接证明刘看山当晚就是咖啡因导致？</b><p>选择一个答案，确认这条专业观点在本案中的证明范围。</p></header><div className="boundary-options"><button className={boundaryAnswer === "can" ? "selected incorrect" : ""} onClick={() => setBoundaryAnswer("can")}><span />能直接证明</button><button className={boundaryAnswer === "cannot" ? "selected correct" : ""} onClick={() => setBoundaryAnswer("cannot")}><span />不能直接证明</button></div>{boundaryAnswer && <div className={`boundary-feedback ${boundaryAnswer === "cannot" ? "correct" : "incorrect"}`}>{boundaryAnswer === "cannot" ? <CheckCircle2 /> : <X />}<p><b>{boundaryAnswer === "cannot" ? "判断正确" : "还不能这样下结论"}</b>{boundaryAnswer === "cannot" ? "它能说明咖啡因影响睡眠的一般规律，但不能单独证明刘看山当晚的个体因果。" : "这是一条一般性专业观点，还需要当晚饮用量、时间及其他变量等本案证据。"}</p></div>}</div>}{round.mode === "research" && <div className="boundary-check"><button className={checked.includes("source") ? "selected" : ""} onClick={() => toggle("source")}><Check />已查看研究来源与样本</button><button className={checked.includes("limit") ? "selected" : ""} onClick={() => toggle("limit")}><Check />相关性不能直接证明本案因果</button></div>}{round.mode === "comments" && <div className="hypothesis"><label>选择一个待查假设</label>{["空调启停", "楼上声音", "宠物活动"].map((item) => <button key={item} className={variable === item ? "selected" : ""} onClick={() => setVariable(item)}>{item}</button>)}</div>}{round.mode === "comparison" && <div className="hypothesis"><label>指出一个仍未控制的变量</label>{["当天情绪", "实际入睡时间", "环境温度"].map((item) => <button key={item} className={variable === item ? "selected" : ""} onClick={() => setVariable(item)}>{item}</button>)}</div>}</section>;
}
